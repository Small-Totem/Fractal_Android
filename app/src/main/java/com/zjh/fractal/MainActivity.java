...
public class MainActivity extends AppCompatActivity {
    ...
    static boolean color_reversal=true;//颜色反转
    static boolean flag_should_load_from_storage =false;//在改日夜模式后为true，是否加载图片,默认false
    static boolean flag_use_data;/*为了实现每次更新信息都暂存数据（用于恢复上次关闭时的参数），使用
    ...
    final static int fractal_id_num=11;//除去-1,一共9个
    final static int iteration_max=8192;//为了防止渲染时间太长才设的限制
    final static double scale_max=4E12;//由于fractal.c那边的限制，超过这个数后很快就不能正常渲染了（待研究）
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        file_path=getFileRoot(this)+"/fractal.png";
        ...
        /*上面是lambda表达式，这是替换之前的
            switch1.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                flag_if_generate_now=isChecked;
            }
        });
         */
    ...
    protected void onStart() {
        ...
        if(flag_should_reload)generate(pixel_times);
        else if(flag_should_load_from_storage){
            ImageView i =findViewById(R.id.fractal);
            Bitmap bitmap = GetLocalBitmap(file_path);
    ...
    public void generate(double quality){
        //当quality大于1会闪退，但没必要改
        pixel_times=quality;
        update_info();

        GenerateFractal(file_path,(int)(screenHeight*pixel_times),(int)(screenWidth*pixel_times),
                center_x,center_y,scale_times*pixel_times,
                fractal_id,get_color_reversal(),generate_mode,iteration_times);
    ...
    public native void GenerateFractal(String path, int screen_height, int screen_width,
                                       double center_x, double center_y, double SCALE_times,
                                       int fractal_id,int color_reversal,int generate_mode,int iteration_times);
    ...
    public void update_info() {
        ...
        String str_reversal     ="反转="+ color_reversal;
        ...
        getSharedPreferences("fractal_settings_Preferences",MODE_PRIVATE)
                .edit().putBoolean("color_reverse_Preference",color_reversal).apply();
        ...
        if(fractal_id==-1){
            getSharedPreferences("fractal_settings_Preferences",MODE_PRIVATE)
                    .edit().putString("fractal_id_Preference",str1[fractal_id_num]).apply();
        }
        else{
            getSharedPreferences("fractal_settings_Preferences",MODE_PRIVATE)
                    .edit().putString("fractal_id_Preference",str1[fractal_id]).apply();
        }

        if(generate_mode==-1){
            getSharedPreferences("fractal_settings_Preferences",MODE_PRIVATE)
                    .edit().putString("generate_mode_Preference",str2[generate_mode_max]).apply();
        }
        else if(generate_mode==-2){
            getSharedPreferences("fractal_settings_Preferences",MODE_PRIVATE)
                    .edit().putString("generate_mode_Preference",str2[generate_mode_max+1]).apply();
        }
        else{
            getSharedPreferences("fractal_settings_Preferences",MODE_PRIVATE)
                    .edit().putString("generate_mode_Preference",str2[generate_mode]).apply();
        }

        SaveData.save_data(center_x, center_y, scale_times, pixel_times,
                generate_now_quality, color_reversal, fractal_id,
                generate_mode,iteration_times,getApplicationContext(),flag_use_data);
    }
    ...
    public void settings(View v) {
        /*
        fractal_id++;
        if(fractal_id==fractal_id_max)fractal_id=0;
        if(flag_if_generate_now)generate(generate_now_quality);
        */
    ...
    public void change_generate_mode(View v) {
        generate_mode++;
        if(generate_mode==generate_mode_max)generate_mode=0;
        if(flag_if_generate_now)generate(generate_now_quality);
    }
